...
expertNumber = os.listdir('Data/')
histoImages = create_images('Data/' + expertNumber[-1]) # create list of HE images
groundTruths = []  # create a set of list of maps, groundTruths[expertNumber][imageNumber]
for h in range(len(expertNumber)-1):
    groundTruths.append(create_images('Data/' + expertNumber[h]))

#count the number of slices per slides per expert 
slideList = [1,2,3,5,6,7]
slicePerSlidePerExpert = []
for j in range(len(expertNumber)-1): #par expert
    slideCounter = [0, 0, 0, 0, 0, 0, 0]
    for i in range(6): #par slide00
        slide = 'slide00'+str(slideList[i])
        for k in range(len(groundTruths[j])): #par slice
            if slide in groundTruths[j][k]:
                slideCounter[i] +=1
